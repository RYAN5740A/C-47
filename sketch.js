...
function setup(){
    ...
    world=engine.world;
    homeBG=createSprite(800,200,windowWidth,windowWidth)
...
// console.log( mousePressedOver(startButton))
    ...
    VerticalProp=createSprite(width/8,height/1-60,30,100)
    VerticalProp.shapeColor="steelblue"
    HorizontalProp=createSprite(width/8+200,height/1-60,100,30)
    HorizontalProp.shapeColor="steelblue"
    LeftAngledProp=createSprite(width/8+400,height/1-60,100,30)
    LeftAngledProp.shapeColor="steelblue"
 ...
 Info()
  ...
  function Info(){
      ...
      InfoBG.addImage(GreyImg)
      InfoBG.alpha=0.5
      text("Click on the Start Butoon on the bottom of you're screen to start",width/2-200,height/8+50)
      text("Read what you have to build which is mentioned in the top then think of an idea and start creating it",width/2-600,height/8+400)
